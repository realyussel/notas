...
class Deepwiki {
	...
	private function fillTemplate() {
		...
		// Ocultar header y footer en modo aula...
		if ($this->request->query[3]) {
			$this->template
				->setPart('header', '')
				->setPart('footer', '');
		} else {
			...
			$part_header = array(
				...
				'<img src="dist/y-dark.svg" alt="" width="40" height="40" class="me-2">',
				'</div>',
				'<nav class="me-auto">',
				...
				'</ul>',
				'</nav>',
				'<a href="../edit" class="btn btn-outline-light text-end">Editor</a>',
				'</div>',
				'</div>',
				'</header>',
			);
			$logout_link = '';
			if (self::AUTH_LOGGED_IN == $this->authenticated) {
				$logout_link = sprintf(' <a class="btn btn-sm btn-outline-secondary" href="%s">Cerrar sesión</a>', $this->uri('_logout'));
			}
			$part_footer = array(
				'<footer class="bd-footer bg-light">',
				'<div class="container">',
				'<p class="m-0 p-3">© yussel.com.mx' . $logout_link . '</p>',
				'</div>',
				'</footer>',
			);
			$this->template
	...
	private function uri($path = null, $absolute = false) {
		...
		if (empty($path)) {
			$uri = rtrim(SITE_URI, '/') . '/index.php?usr=' . $this->request->query[2] . '&nb=' . $this->request->query[1] . ($this->request->query[3] ? '&aula' : '');
		} else {
			if ($path == '!') {
				...
				$uri = rtrim(SITE_URI, '/') . '/index.php?usr=' . $this->request->query[2] . '&nb=' . $this->request->query[1] . '&p=' . trim($path, '/') . ($this->request->query[3] ? '&aula' : '');
			}
		}
		if ($absolute) {
...
class DeepwikiRequest {
	...
	public function capture() {
		...
		$this->query[3] = array_key_exists('aula', $_GET);
		return $this;
	}
}

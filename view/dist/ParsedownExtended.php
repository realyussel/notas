...
class ParsedownExtended extends DynamicParent {
	...
	protected function blockHeader($line) {
		...
		if (!empty($block)) {
			...
			$id = $block['element']['attributes']['id'] ?? $this->createAnchorID();
	...
	*/
	...
	protected function checkboxUnchecked($text) {
		...
		return '<input type="checkbox" disabled /><label>' . $this->format($text) . '</label>';
	}
	...
	protected function checkboxChecked($text) {
		...
		return '<input type="checkbox" checked disabled /><label>' . $this->format($text) . '</label>';
	}
	 ...
	 * ASCII.
	 */
	protected function createAnchorID(): string {
		return $this->incrementAnchorId('title');
	}
